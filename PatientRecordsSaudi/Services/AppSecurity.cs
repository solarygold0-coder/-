...
namespace PatientRecordsSaudi.Services
{
    ...
    public sealed class AppSecurity
    {
        ...
        private SecurityStore LoadStore() { SecurityStore s = JsonSerializer.Deserialize<SecurityStore>(File.ReadAllText(authPath, Encoding.UTF8), JsonOptions); if (s == null || s.Version != 2 || s.Users == null || s.Users.Count == 0) throw new InvalidDataException("ملف الحماية غير صالح."); return s; }
        private void SaveStore(SecurityStore store) { AtomicWrite(authPath, JsonSerializer.Serialize(store, JsonOptions)); }
