...
namespace PatientRecordsSaudi.UI
{
    public sealed class MainForm : Form
    {
        ...
        private readonly CheckBox requireLogin = new CheckBox { Text = "طلب تسجيل الدخول عند التشغيل (تُطبّق صلاحية كل حساب: مدير/موظف/قراءة فقط)", AutoSize = true, Font = UiKit.BoldFont, Margin = new Padding(8, 12, 8, 8) };
        ...
        private ReminderForm activeReminder; private Form lockedReminder; private DateTime? lastLockedReminderAt; private bool forceExit, locked; private readonly InactivityFilter activity = new InactivityFilter();
        ...
        public MainForm(AppDatabase database, BackupService backupService, AppSecurity security, SecuritySession session)
        {
            ...
            Application.AddMessageFilter(activity); idleTimer.Interval = 30000; idleTimer.Tick += delegate { if (security.IsLoginRequired && !locked && DateTime.Now - activity.LastActivity > TimeSpan.FromMinutes(15)) LockApplication(); }; idleTimer.Start();
        ...
        private TabPage BuildSettings()
        {
            ...
            AddSetting(body, "اسم المنشأة", clinicName); AddSetting(body, "هاتف المنشأة", clinicPhone); AddSetting(body, "عنوان المنشأة", clinicAddress); AddSetting(body, "بداية الدوام", workStart); AddSetting(body, "نهاية الدوام", workEnd); AddSetting(body, "نسخة تلقائية كل (ساعة)", backupHours); AddSetting(body, "حماية تسجيل الدخول", requireLogin);
            ...
            Button save = UiKit.Button("حفظ الإعدادات", SaveSettings, false), restore = UiKit.Button("استعادة نسخة", RestoreBackup, true), export = UiKit.Button("تصدير قائمة CSV", ExportCsv, false), users = UiKit.Button("حسابات المستخدمين", ManageUsers, false), closures = UiKit.Button("الإجازات وأيام الإغلاق", ManageClosures, false), recycle = UiKit.Button("المحذوفات", OpenRecycleBin, false);
            ...
            Button changePassword = UiKit.Button("تغيير كلمة المرور", ChangePassword, false); changePassword.Enabled = session.IsAdmin && security.HasUserAccount(session.Username);
            actions.Controls.Add(save); actions.Controls.Add(createBackup); actions.Controls.Add(restore); actions.Controls.Add(export); actions.Controls.Add(users); actions.Controls.Add(closures); actions.Controls.Add(recycle); actions.Controls.Add(changePassword); actions.Controls.Add(audit); actions.Controls.Add(exportAudit); actions.Controls.Add(purgeAttachments);
        ...
        private void SaveSettings(object s, EventArgs e)
        {
            ...
            try
            {
                ...
                database.SaveSettings(set); bool loginChanged = security.IsLoginRequired != requireLogin.Checked; security.SetLoginRequired(session, requireLogin.Checked); Text = "نظام إدارة سجلات المراجعين - " + set.ClinicName;
                if (loginChanged && requireLogin.Checked) MessageBox.Show("تم تفعيل تسجيل الدخول باستخدام حسابات المستخدمين التي أنشأتها. لا يوجد حساب أو كلمة مرور افتراضية.", "تم تفعيل الحماية", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.RtlReading | MessageBoxOptions.RightAlign);
                else MessageBox.Show("تم حفظ الإعدادات.", "تم", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex) { UiKit.ShowError(ex.Message); }
        }
        ...
        private void ConfigureNotification()
        {
            ...
            try { using (RegistryKey run = Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run", true)) if (run != null) { run.DeleteValue("SaudiPatientRecords", false); run.SetValue("SaudiPatientRecordsV5", "\"" + Application.ExecutablePath + "\""); } } catch { }
        }
        private void CheckReminders()
        {
            try
            {
                if (activeReminder != null && !activeReminder.IsDisposed) return;
                DateTime now = DateTime.Now, appointmentLimit = now.AddDays(2), taskLimit = now.AddMinutes(5), missedTasks = now.AddDays(-7);
                List<Appointment> appointments = database.GetUnnotifiedAppointments(now, appointmentLimit, 50); List<PatientTask> tasks = database.GetUnnotifiedTasks(missedTasks, taskLimit, 50);
                if (appointments.Count == 0 && tasks.Count == 0) return;
                if (locked || !Visible)
                {
                    if ((lockedReminder != null && !lockedReminder.IsDisposed) || (lastLockedReminderAt.HasValue && now - lastLockedReminderAt.Value < TimeSpan.FromMinutes(30))) return;
                    lockedReminder = new LockedReminderForm(appointments.Count, tasks.Count, delegate
                    {
                        if (!UnlockAndShow()) return; Form alert = lockedReminder; lockedReminder = null; if (alert != null && !alert.IsDisposed) alert.Close();
                    });
                    lockedReminder.FormClosed += delegate(object sender, FormClosedEventArgs e) { if (ReferenceEquals(lockedReminder, sender)) lockedReminder = null; };
                    lastLockedReminderAt = now; lockedReminder.Show(); return;
                }
                activeReminder = new ReminderForm(appointments, tasks, delegate(Guid patientId)
                {
                    Patient patient = database.GetPatient(patientId); if (patient != null) OpenPatient(patient); else UiKit.ShowError("تعذر العثور على ملف المراجع.");
                });
                activeReminder.FormClosed += delegate { activeReminder.Dispose(); activeReminder = null; };
                activeReminder.Show(this);
                foreach (Appointment a in appointments) database.MarkAppointmentNotified(a.Id); foreach (PatientTask t in tasks) database.MarkTaskNotified(t.Id);
            }
            catch (Exception ex) { try { database.Audit("خطأ في نافذة التنبيهات", "System", "reminders", null, ex.Message); database.Checkpoint(); } catch { } }
        }

        private void LockApplication() { if (!security.IsLoginRequired || locked) return; locked = true; if (activeReminder != null && !activeReminder.IsDisposed) activeReminder.Close(); Hide(); notify.BalloonTipTitle = "تم قفل البرنامج"; notify.BalloonTipText = "تم القفل تلقائيًا لحماية البيانات. انقر لفتح البرنامج."; notify.ShowBalloonTip(5000); }
        ...
        {
            if (!forceExit && e.CloseReason == CloseReason.UserClosing) { e.Cancel = true; locked = security.IsLoginRequired; if (activeReminder != null && !activeReminder.IsDisposed) activeReminder.Close(); Hide(); notify.BalloonTipTitle = "البرنامج يعمل في الخلفية"; notify.BalloonTipText = "ستستمر تنبيهات المواعيد والمهام. استخدم أيقونة البرنامج بجانب الساعة للفتح أو الإنهاء."; notify.ShowBalloonTip(7000); return; }
