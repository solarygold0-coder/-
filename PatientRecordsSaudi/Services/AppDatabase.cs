...
namespace PatientRecordsSaudi.Services
{
    public sealed class AppDatabase : IDisposable
    {
        ...
        {
            RequireWrite(); db.BeginTrans(); try { Appointment old = GetAppointment(a.Id); if (old == null) throw new InvalidOperationException("الموعد غير موجود."); ValidateAppointmentAvailability(a); if (old.StartsAt != a.StartsAt || (old.Status != a.Status && a.Status != "ملغي")) a.ReminderNotifiedAt = null; a.UpdatedAt = DateTime.Now; db.GetCollection<Appointment>("appointments").Update(a); RecalculateLastVisit(a.PatientId); if (old.PatientId != a.PatientId) RecalculateLastVisit(old.PatientId); AuditInternal("تعديل موعد", "Appointment", a.Id.ToString(), a.FileNumber, a.Title); db.Commit(); db.Checkpoint(); } catch { db.Rollback(); throw; }
        }
        ...
        public void ValidateAppointmentAvailability(Appointment a)
        {
            ...
            ValidateAppointmentSlot(a.StartsAt, a.DurationMinutes, a.Status, a.Id);
        }

        private void ValidateAppointmentSlot(DateTime startsAt, int durationMinutes, string status, Guid ignoredAppointmentId)
        {
            if (!SaudiValidation.IsOfficialWorkingDay(startsAt)) throw new InvalidOperationException("لا يمكن حجز موعد يوم الجمعة أو السبت.");
            if (db.GetCollection<ClosureDate>("closures").Exists(x => x.Date == startsAt.Date)) throw new InvalidOperationException("هذا اليوم مسجل كإجازة أو يوم إغلاق للمنشأة.");
            AppSettings s = GetSettings(); int start = startsAt.Hour * 60 + startsAt.Minute, endMinutes = start + durationMinutes; if (start < s.WorkDayStartMinutes || endMinutes > s.WorkDayEndMinutes) throw new InvalidOperationException("الموعد خارج ساعات الدوام المحددة في الإعدادات.");
            if (status == "ملغي") return;
            DateTime end = startsAt.AddMinutes(durationMinutes), dayStart = startsAt.Date, dayEnd = dayStart.AddDays(1);
            Appointment conflict = db.GetCollection<Appointment>("appointments").Find(x => x.StartsAt >= dayStart && x.StartsAt < dayEnd && !x.IsDeleted).FirstOrDefault(x => x.Id != ignoredAppointmentId && x.Status != "ملغي" && x.StartsAt < end && x.StartsAt.AddMinutes(x.DurationMinutes) > startsAt);
        ...
        {
            if (durationMinutes <= 0 || durationMinutes > 12 * 60) throw new ArgumentOutOfRangeException("durationMinutes", "مدة الموعد غير صحيحة.");
            ...
            for (int i = 0; i < 365 * 24 * 4; i++)
            {
                ...
                try { ValidateAppointmentSlot(candidate, durationMinutes, "مؤكد", Guid.Empty); return candidate; } catch (AppointmentConflictException) { candidate = candidate.AddMinutes(15); }
            }
        ...
        public void UpdateTask(PatientTask t) { RequireWrite(); ValidateTask(t); PatientTask old = db.GetCollection<PatientTask>("tasks").FindById(t.Id); if (old == null) throw new InvalidOperationException("المهمة غير موجودة."); if (old.DueAt != t.DueAt || (old.IsCompleted && !t.IsCompleted)) t.ReminderNotifiedAt = null; t.UpdatedAt = DateTime.Now; db.GetCollection<PatientTask>("tasks").Update(t); Audit("تعديل مهمة", "Task", t.Id.ToString(), t.FileNumber, t.Title); db.Checkpoint(); }
